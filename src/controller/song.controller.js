...
class SongController {
    ...
    async updateSong(ctx, next) {
        try {
            // song_singerid不用修改
            const { song_id,song_songname,song_rank,song_list_id } = ctx.request.body
            let { song_imgpath, song_filepath, song_lycpath } = ctx.request.files
            song_imgpath = APP_DURL + path.basename(song_imgpath.filepath)
            song_filepath = APP_DURL + path.basename(song_filepath.filepath)
            song_lycpath = path.basename(song_lycpath.filepath)
            ...
            if (await songService.updateSong({ song_id,song_songname,song_rank,song_singerid,song_list_id,song_imgpath, song_filepath, song_lycpath })) {
                return ctx.body = {
                    code: 200,
                    message: '修改歌曲信息成功',
                    data: ''
